function [node2,triangle2,subBoundary] = NodeSorting2timed(node,triangle)
...
if type == 1
    ...
    find_time = toc 
    disp(sprintf('Time to find boundary in mesh was %f',find_time));
    keyboard
    %% Look for the sub-boundaries
    %isBoundary = isBoundarySave;
    tic
    % TWITZEL: this is a vector of all boundary nodes
    nodeOnBoundary = unique(isBoundary(:));
    % TWITZEL: now he is finding all the subboundaries. This code is not
    % assuming a manifold, so he is not looking for loops per se, also
    % he is following all neighbors of a vertex and checks whether they are
    % part of the boundary, which is crazy
    if not(isempty(nodeOnBoundary))
        ...
        sub_time = toc
        disp(sprintf('Time to find sub boundaries was %f',sub_time));

        %% Then we create a new node matrix with the border node on top of it
        % TWITZEL: Okay, this is a pretty simple task, just reorder the
        % node vector, and then remap the triangles.
        isBoundary = isBoundarySave;
